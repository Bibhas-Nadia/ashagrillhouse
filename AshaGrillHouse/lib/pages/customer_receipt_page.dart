...
class _CustomerReceiptPageState extends State<CustomerReceiptPage> {
  ...
  bool _sendSmsOnSave = true; // 🟢 নতুন ভ্যারিয়েবল (ডিফল্ট ট্রু রাখা হলো)
  ...
  void initState() {
    super.initState();
    _speech = stt.SpeechToText(); // 🎤 এখানে যুক্ত করুন
  ...
  // 🟢 Add this method inside your _CustomerReceiptPageState class
  Future<String> _getDir() async {
    ...
    try {
      ...
      debugPrint("❌ Error finding SD card directories: $e");
    }
    ...
    if (sdCardRootPath != null) {
      ...
      try {
        ...
        debugPrint("⚠️ OS Blocked SD Card 'Pictures' access: $e");
      }
    }
  ...
  Future<void> _loadSavedCalculation() async {
    ...
    String? savedJson = await DBHelper.getReceiptCalculation(widget.customer.id!);
    if (savedJson != null && savedJson.isNotEmpty) {
      try {
        final decoded = jsonDecode(savedJson);

        if (decoded is Map && decoded.containsKey('savedBills')) {
          _savedBills = List<Map<String, dynamic>>.from(decoded['savedBills']);
          List<dynamic> draftList = decoded['currentDraft'] ?? [];
          _appData = draftList.map((e) => ReceiptItem.fromMap(e)).toList();
        } else if (decoded is List) {
          _appData = decoded.map((e) => ReceiptItem.fromMap(e)).toList();
          _savedBills = [];
        }
    ...
    // 🟢 ডাটাবেস থেকে লাইভ বকেয়া (Due) চেক করার কোড
    ...
    setState(() {
      if (_savedBills.isEmpty && _appData.isEmpty) {
        ...
        // 🟢 কোনো বিল না থাকলে সরাসরি ওপেন হওয়ার সময় পূর্বের বকেয়া অটোমেটিক ইনজেক্ট হবে
        if (realLiveDue > 0) {
      ...
      widget.customer.due = realLiveDue.toString(); // উইজেট ডিউ সিঙ্ক
      _isLoading = false;
    });
  }
        ...
        children: [
          ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              // 🟢 লাইভ বকেয়া (Due) ডাটাবেস থেকে রিড করা হবে
              double realLiveDue = 0.0;
              try {
                final dbClient = await DBHelper.db;
                final List<Map<String, dynamic>> res = await dbClient.query(
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              IconButton(
                                ...
                                onPressed: () async {
                                  // 🔴 STRONG CONFIRMATION MESSAGE ADDED HERE 🔴
                                  bool confirm = await _confirmAction("আগের বিল ডিলিট করবেন?", "এই বিলটি চিরতরে মুছে যাবে এবং আর ফিরে পাওয়া যাবে না। আপনি কি নিশ্চিত?");
                                  if (confirm) {
                                    setState(() {
                                      _savedBills.removeAt(index);
                                      if (_editingBillId == bill['billId'].toString()) {
                                        _editingBillId = null;
                                        _editingBillDate = "";
                                        _appData = [];
                                      }
  ...
  // =========================================================================
  // BOTTOM SHEETS WITH CUSTOM INPUT
  // =========================================================================
  ...
  void _showAddItemSheet() {
    ...
    bool isCustom = false; // Tracks if user selected "Others"
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // 🔴 NEW: CUSTOM ITEM INPUT FIELD WITH MIC SUPPORT
                      if (isCustom) ...[
                        const SizedBox(height: 16),
                        TextField(
                              ...
                              onPressed: () async {
                                if (_isListening) {
                                  ...
                                  if (available) {
                                    ...
                                    _speech.listen(
                                      localeId: "bn_IN", // বাংলা ভয়েস টাইপিং এর জন্য
                                      onResult: (result) {
                                        setModalState(() {
  ...
  Future<void> _exportAndSaveFinalReceipt() async {
    ...
    try {
      ...
      // final directory = await getApplicationDocumentsDirectory();
      // String filePath = '${directory.path}/receipt_${widget.customer.id}_${DateTime.now().millisecondsSinceEpoch}.png';
      ...
      // 🟢 Use the custom SD card / Internal Storage directory logic instead
      final dirPath = await _getDir();
      String filePath = '$dirPath/receipt_${widget.customer.id}_${DateTime.now().millisecondsSinceEpoch}.png';

      await File(filePath).writeAsBytes(pngBytes);


      List<String> userImages = widget.customer.images.split(",").where((s) => s.trim().isNotEmpty).toList();
      userImages.add(filePath);
      ...
      // --- SEND SMS AND LOG TO DB (এই কন্ডিশনটি এভাবে আপডেট করুন) ---
      ...
      Map<String, dynamic> completeBillObj = {
        "billId": _editingBillId ?? DateTime.now().millisecondsSinceEpoch.toString(),
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            //   label: const Text("ড্যাশবোর্ডে ফিরে যান", style: TextStyle(color: Colors.white)),
            // )
                    ...
                    const Padding(
                      ...
                      Table(
                        columnWidths: {
                          ...
                          2: _isExporting ? const FixedColumnWidth(0) : const IntrinsicColumnWidth(),
                        },
                          ...
                          children: [
                            TableRow(
                              ...
                              children: [
                                ...
                                Padding(padding: const EdgeInsets.all(10.0), child: Text(_isExporting ? "" : "ডিলিট", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey.shade800, fontSize: 13))),
                              ]
                            ),
          ...
          if (billedItems.isNotEmpty)
            Column(
              ...
              children: [
                                      ...
                                      // 🟢 নতুন যোগ করা SMS চেকবক্স (বাটনের ঠিক ওপরে)
                CheckboxListTile(
                  title: const Text(
                ...
                const SizedBox(height: 8), // বাটনের সাথে গ্যাপ
