...
// import '../db_helper.dart'; // 👈 Import DBHelper instead of SharedPreferences
//
...
//
//     try {
//       // 👈 Fetch directly from the single-row database table
...
//   }
//
//   // 🎨 Updated Helper to check the database "status" and "issue" values
...
//       if (_issue.contains('network_issue')) {
//         return {
...
//
//             // Shows you exactly what caused the error from the database 'issue' column
//             Container(
...
//               ),
//               child: Text(
//                 "Status Info: $_issue", // 👈 Now displays the exact issue string
//                 textAlign: TextAlign.center,
...
//               child: Row(
//                 children: [
//                   Icon(Icons.info_outline_rounded, color: Colors.indigo.shade400),
...
class _BackupStatusPageState extends State<BackupStatusPage> {
  bool _isLoading = true;
  bool _isBackupEnabled = false; // New Toggle State Variable
  String _backupDate = "Never";
  String _status = "none";
  String _issue = "No data";
  ...
  Future<void> _loadBackupStatus() async {
    ...
    try {
      // Load the toggle settings from the new settings table
      final bool enabledSetting = await DBHelper.isBackupEnabled();

      // Fetch status directly from the single-row database table
      final Map<String, dynamic>? backupData = await DBHelper.getBackupStatus();
  ...
  // Updated Helper to handle custom layout visuals based on toggle & execution status
  Map<String, dynamic> _getStatusVisuals() {
    // If the user completely disabled backups, override visuals first
    if (!_isBackupEnabled) {
      return {
        'color': Colors.grey.shade600,
    ...
    return Scaffold(
      ...
      : Padding(
        ...
        child: Column(
          ...
          children: [
            // ==========================================
            // ⚙️ BACKUP ACTIVATION SWITCH CARD
            // ==========================================
            Card(
              color: Colors.white,
              elevation: 0,
            ...
            // Shows exact error information from database string
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: Colors.grey.shade200,
