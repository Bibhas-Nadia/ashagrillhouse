...
class _ImportPageState extends State<ImportPage> {
  ...
  // ==========================================
  // ☁️ IMPORT FROM GOOGLE SHEETS & DRIVE
  // ==========================================
  Future<void> _importFromGoogleSheetAndDrive() async {
    setState(() {
      _isImportingFromDrive = true;
      _driveSyncStatus = "Step 1/4: Fetching Google Sheets...";
    });

    try {
      final config = await DBHelper.getConfig();
      String scriptUrl = config?['googleScriptUrl']?.toString().trim() ?? "";
      if (scriptUrl.isEmpty) throw Exception("Google Script URL is missing in Settings.");

      final client = http.Client();
      http.Response scriptResponse;
      try {
        final request = http.Request('POST', Uri.parse(scriptUrl))
        ..headers["Content-Type"] = "application/json"
        ..body = jsonEncode({
          "apiKey": "89faa837458e8dbc66de416fa04f9466b462d2b77a2b3065d9d6e7cc58288777",
          "action": "fetch_full_database"
        })
        ..followRedirects = false;

        final streamedResponse = await client.send(request);
        scriptResponse = await http.Response.fromStream(streamedResponse);

        if (scriptResponse.statusCode == 302 || scriptResponse.statusCode == 303) {
          String? redirectUrl = scriptResponse.headers['location'];
          if (redirectUrl != null) scriptResponse = await http.get(Uri.parse(redirectUrl));
        }
      } finally {
        client.close();
      }

      if (scriptResponse.statusCode != 200) throw Exception("Failed to fetch database.");

      setState(() => _driveSyncStatus = "Step 2/4: Parsing Data...");
      Map<String, dynamic> jsonData = jsonDecode(scriptResponse.body);
      if (jsonData['status'] == 'error') throw Exception(jsonData['message']);
      Map<String, dynamic> dbData = jsonData['database'];

      setState(() => _driveSyncStatus = "Step 3/4: Updating Database...");
      Directory customersDir = await _getTargetDirectory("customers");
      Directory measurementsDir = await _getTargetDirectory("measurements");
      final db = await DBHelper.db;
      List<String> tables = ['customers', 'transactions', 'tempcustomer', 'measurements', 'expenses', 'orders','messages','pins','price_agreements','receipt_calculations','expense_keywords'];
      ...
      for (String table in tables) {
        if (dbData.containsKey(table)) {
          await db.delete(table);
          for (var row in dbData[table]) {
            if (row.containsKey('images') && row['images'] != null && row['images'].toString().isNotEmpty) {
              List<String> oldPaths = row['images'].toString().split(',');
              List<String> newPaths = [];
              for (String oldP in oldPaths) {
                if (oldP.trim().isEmpty) continue;
                String fName = p.basename(oldP.trim());
                String targetDirPath = oldP.toLowerCase().contains('/measurements/') || oldP.toLowerCase().contains('/mesurements/')
                ? measurementsDir.path : customersDir.path;
                String newTargetLocalPath = "$targetDirPath/$fName";
                newPaths.add(newTargetLocalPath);
                expectedImageLocations[fName] = newTargetLocalPath;
              }
      ...
      setState(() => _driveSyncStatus = "Step 4/4: Connecting to Drive...");
      GoogleSignInAccount? account = await _googleSignIn.signIn();
      if (account == null) throw Exception("Google Sign-In Cancelled. Images skipped.");
      ...
      if (folderList.files == null || folderList.files!.isEmpty) throw Exception(".AshaGrillHouse folder not found on Drive.");
      String mainFolderId = folderList.files!.first.id!;

      Future<int> downloadMissingImages(String folderName) async {
        int count = 0;
        var driveFolderReq = await driveApi.files.list(
          q: "mimeType='application/vnd.google-apps.folder' and name='$folderName' and '$mainFolderId' in parents and trashed=false",
          $fields: "files(id)"
        ...
        do {
          ...
          if (fileList.files != null) {
            for (var f in fileList.files!) {
              if (f.id == null || f.name == null) continue;
              String fName = f.name!;
              String targetPath = expectedImageLocations[fName] ?? "${folderName == 'measurements' ? measurementsDir.path : customersDir.path}/$fName";

              File localFile = File(targetPath);
              if (!localFile.existsSync()) {
                // 🔴 SHOW EXACT IMAGE BEING DOWNLOADED IN UI
                setState(() => _driveSyncStatus = "Downloading: $fName");
                var imgMedia = await driveApi.files.get(f.id!, downloadOptions: drive.DownloadOptions.fullMedia) as drive.Media;
                var sink = localFile.openWrite();
                await imgMedia.stream.pipe(sink);
                await sink.close();
      ...
      setState(() => _driveSyncStatus = "Checking customers folder...");
      int downloadedCustomers = await downloadMissingImages("customers");

      setState(() => _driveSyncStatus = "Checking measurements folder...");
      int downloadedMeasurements = await downloadMissingImages("measurements");

      _showSuccessMsg("Import Complete! Downloaded ${downloadedCustomers + downloadedMeasurements} missing images.");

    } catch (e) {
      _showErrorMsg("Sync failed: $e");
    } finally {
      setState(() => _isImportingFromDrive = false);
    }
  }
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ☁️ GOOGLE SCRIPT & DRIVE IMPORT CARD
            _buildImportCard(
              title: "Restore from Sheets & Drive",
              description: "Fetches your live database from Google Sheets and automatically downloads missing images into SD Card.",
              icon: Icons.cloud_sync_rounded,
              color: Colors.blueAccent,
              buttonText: "FETCH FROM SCRIPT",
              isLoading: _isImportingFromDrive,
              statusText: _driveSyncStatus, // 🔴 PASS STATUS HERE
              onTap: _importFromGoogleSheetAndDrive,
            ),
  ...
  Widget _buildImportCard({
    ...
    required String buttonText,
    required bool isLoading,
    String? statusText, // 🔴 ADD THIS PARAMETER
    required VoidCallback onTap,
  }) {
    return Container(
      decoration: BoxDecoration(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    // 🔴 UPDATED BUTTON CHILD TO SHOW PROGRESS TEXT
                    child: isLoading
                    ? Row(
                      mainAxisAlignment: MainAxisAlignment.center,
                      children: [
