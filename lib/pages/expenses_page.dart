...
class _ExpensesPageState extends State<ExpensesPage> {
  ...
  // 🔄 FILTER, SORT & SUM CALCULATION (Includes Search!)
  void _applyFilterAndSum() {
    List temp;
    ...
    // Sorts the list from newest date (top) to oldest date (bottom)
    temp.sort((a, b) {
      DateTime dateA;
      DateTime dateB;
      try {
        dateA = DateFormat('dd-MM-yyyy hh:mm a').parse(a['date']);
      } catch (_) {
        dateA = DateTime.fromMillisecondsSinceEpoch(0);
      }
      try {
        dateB = DateFormat('dd-MM-yyyy hh:mm a').parse(b['date']);
      } catch (_) {
