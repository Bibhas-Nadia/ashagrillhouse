...
import '../backup_service.dart'; // Add this line
...
class _MeasurementDetailPageState extends State<MeasurementDetailPage> {
  ...
  Future<void> _handleMenuAction(String action) async {
    ...
    if (action == 'add_customer') {
      ...
      // 🟢 Trigger Backup for new customer creation
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () async {
                            ...
                            // 1. Trigger Cloud Backup for newly created customer & advance transaction
                            BackupService.autoBackupOnAppOpen();
                            ...
                            if (sendSMS && widget.measurement.phone.isNotEmpty) {
                              ...
                              if (status == "active") {
                                msg = "Dear ${widget.measurement.name}\nWe have received your advance payment of Rs. $amount. Thank you!\n\n- Asha Grill House\nView details: https://ashagrillhouse.github.io/site/bill.html?q=${widget.measurement.phone.replaceAll(RegExp(r'\D'), '').substring(widget.measurement.phone.replaceAll(RegExp(r'\D'), '').length - 10)}";
                              } else {
                                msg = "Dear ${widget.measurement.name}\nWe have received your advance payment of Rs. $amount. Thank you!\n\n- Asha Grill House";
                              }
                              ...
                              "${DateTime.now().hour}:${DateTime.now().minute}";
                                ...
                                // 2. Save message to Local DB FIRST
                                if (newCust.id != null) {
                                  await DBHelper.insertMessage(newCust.id!, msg, nowStr);
                                } else {
