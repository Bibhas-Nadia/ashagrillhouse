  ...
  load() async {
    ...
    transactions.sort((a, b) {
      if (sortType == "new" || sortType == "old") {
        ...
        if (sortType == "new") {
          return dateB.compareTo(dateA); // Newest to Oldest (Top to Bottom)
        } else {
...
//   }
  ...
  addTransaction() async {
    ...
    if (sendSMS) {
      ...
      if (phone.isNotEmpty && phone.length >= 10) {
        ...
        if (status == "active") {
          // Message for Active Status
          msg = "Dear ${widget.customer.name}\nPayment Rs. $cleanEntered has received.\nYour ${newDue < 0 ? 'Advance Balance is Rs. $cleanAdvance' : 'Remaining Due is Rs. $cleanDue'}\n\n- Asha Grill House\nView details: https://ashagrillhouse.github.io/site/bill.html?q=${phone.replaceAll(RegExp(r'\D'), '').substring(phone.replaceAll(RegExp(r'\D'), '').length - 10)}";
        }
        else {
          // 3. Message for Inactive or any other status
          msg = "Dear ${widget.customer.name}\nPayment Rs. $cleanEntered has received.\nYour ${newDue < 0 ? 'Advance Balance is Rs. $cleanAdvance' : 'Remaining Due is Rs. $cleanDue'}\n\n- Asha Grill House";
        ...
        "${DateTime.now().hour}:${DateTime.now().minute}";
              ...
              // 1. Save message to Local DB FIRST
              await DBHelper.insertMessage(widget.customer.id, msg, nowStr);

              // 2. Trigger Cloud Backup in the background
      ...
      // ✅ Trigger Backup even if the user chose NOT to send an SMS
      BackupService.autoBackupOnAppOpen();
    }
  ...
  deleteTx(int id, String amountValue) async {
    ...
    // 🟢 Trigger Backup when a transaction is deleted
    BackupService.autoBackupOnAppOpen();
