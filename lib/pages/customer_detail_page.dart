...
class _CustomerDetailPageState extends State<CustomerDetailPage> {
  ...
  Map<String, dynamic> _dueHistory = {}; // 🔴 Add this line
  bool _showDueHistory = false; // 🔴 Add this line for the dropdown toggle
  bool _showMessageHistory = false; // 🔴 Add this line for the message history dropdown toggle
  ...
  String _formatDateString(String dateString) {
    ...
    try {
      // Try standard Dart parsing first (for ISO8601 strings)
      DateTime dt = DateTime.parse(dateString);
      return DateFormat('dd MMM yyyy, hh:mm a').format(dt);
    } catch (e) {
      try {
        // If standard fails, parse your specific format: "5-6-2026 19:36"
        DateTime dt = DateFormat('d-M-yyyy HH:mm').parse(dateString);
        return DateFormat('dd MMM yyyy, hh:mm a').format(dt);
      } catch (e2) {
  ...
  Future<Map<String, dynamic>?> _fetchFullReceiptDetails(String targetBillId) async {
    try {
      print("🔍 [DEBUG] Fetching exact receipt. Customer ID: ${widget.c.id}, Row/Bill ID: $targetBillId");

      // 1. Fetch exactly the one row from the database
      String? rawJsonStr = await DBHelper.getSpecificReceiptCalculation(widget.c.id!, targetBillId);

      if (rawJsonStr == null || rawJsonStr.trim().isEmpty) {
        print("❌ [DEBUG] Database returned NULL for Bill ID: $targetBillId");
        return null;
      }

      // 2. It's a clean, single JSON object now. Decode it directly.
      Map<String, dynamic> targetBill = jsonDecode(rawJsonStr);

      // 3. Ensure items list is parsed
      if (targetBill['items'] is String) {
        targetBill['items'] = jsonDecode(targetBill['items']);
      }
      List itemsList = targetBill['items'] ?? [];

      // 4. Calculate total purely from this specific bill's items
      double calculatedTotal = 0.0;
      for (var item in itemsList) {
        calculatedTotal += double.tryParse(item['totalPrice']?.toString() ?? '0') ?? 0.0;
      }

      // 5. Attach required UI fields
      targetBill['billId'] = targetBillId;
      targetBill['totalAmount'] ??= calculatedTotal;
      targetBill['grandTotal'] ??= calculatedTotal;
      targetBill['customerName'] ??= (widget.c.name ?? '');
      targetBill['date'] ??= targetBill['date'] ?? DateTime.now().toIso8601String();

      print("✅ [DEBUG] Successfully parsed single receipt. Items: ${itemsList.length}, Total: $calculatedTotal");
      return targetBill;

    } catch (e) {
      print("❌ [DEBUG] Error parsing receipt JSON: $e");
    }
    return null;
  }
  ...
  // Check if item string represents a receipt
  bool _isReceipt(String item) => item.startsWith('<receipt>');



  // Parse bill ID, amount, and date from receipt format: <receipt>billId&amount&dateStamp</receipt>
  Map<String, String> _parseReceiptData(String item) {
    // Remove both opening and closing tags
    String content = item.replaceAll('<receipt>', '').replaceAll('</receipt>', '');
    List<String> parts = content.split('&');
    return {
      'billId': parts.isNotEmpty ? parts[0] : '',
      'amount': parts.length > 1 ? parts[1] : '0',
  ...
  // --- REPLACEMENT RECEIPT DIALOG FUNCTION (FULL SCREEN ZOOM EFFECT) ---
  void _showReceiptDialog(BuildContext context, String receiptItem) {
    final basicData = _parseReceiptData(receiptItem);
    String billId = basicData['billId'] ?? '';
    String basicAmountStr = basicData['amount'] ?? '0';
    String fallbackDate = basicData['date'] ?? '';

    debugPrint(fallbackDate);

    showDialog(
      context: context,
      builder: (_) => Dialog(
        backgroundColor: Colors.black,
        insetPadding: EdgeInsets.zero, // Removes the 4-side gap for full-screen zoom
        shape: const RoundedRectangleBorder(borderRadius: BorderRadius.zero),
        child: Stack(
          children: [
            // Full-screen viewport allowing unclipped zooming across the screen
            SizedBox(
              ...
              child: InteractiveViewer(
                ...
                minScale: 0.5,
                maxScale: 6.0,
                clipBehavior: Clip.none, // Allows zooming beyond bounds like normal images
                child: Center(
                  child: Container(
                    ...
                    child: FutureBuilder<Map<String, dynamic>?>(
                      future: _fetchFullReceiptDetails(billId),
                      builder: (context, snapshot) {
                        ...
                        if (snapshot.hasError || snapshot.data == null) {
                          return SizedBox(
                            ...
                            child: Center(
                              child: Text(
                                "Receipt #$billId not found in records.",
                        ...
                        final billData = snapshot.data!;
                         //debugPrint("XXXXXXXXXXXXXXXXXXXX "+ billData['date']);
                        List<dynamic> items = billData['items'] ?? [];
                        // rawDate = billData['date'] ?? fallbackDate;
                        String billDate = fallbackDate; // Formats the actual creation date
                        ...
                        return Column(
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                const Icon(Icons.receipt_long_rounded, color: Colors.teal, size: 32),
                                Text("Bill #$billId", style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.teal)),
                              ],
                            ),
                            const Divider(height: 20, thickness: 1.5),
                            Text("Customer: ${widget.c.name}", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)),
                            Text("Date: $billDate", style: const TextStyle(fontSize: 12, color: Colors.grey)),
                            ...
                            const Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
                              children: [
                                Text("ITEM DETAILS", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.teal)),
                                Text("AMOUNT", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.teal)),
                              ],
                            ),
                            ...
                            Flexible(
                              child: SingleChildScrollView(
                                child: Column(
                                  ...
                                  double itemPrice = double.tryParse(item['totalPrice']?.toString() ?? '0') ?? 0.0;
                                  String historyText = item['history']?.toString() ?? '';

                                  String qty = item['prePriceQty']?.toString() ?? '';
                                  String unit = item['originalUnit']?.toString() ?? '';
                                        ...
                                        if (historyText.isNotEmpty) ...[
                                          ...
                                          Container(
                                            ...
                                            decoration: BoxDecoration(
                                              ...
                                              border: Border.all(color: Colors.green.shade50, width: 0.5),
                                            ),
                                            child: Row(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              children: [
                            ...
                            Container(
                              ...
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  const Text("Total Bill Amount", style: TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                                  Text(
                                    "${_formatMoney(double.tryParse(basicAmountStr) ?? 0.0)} টাকা",
  ...
  // 🔴 ADD THIS FUNCTION to handle history deletion with a confirm dialog
  Future<void> _deleteDueHistory(String timestampKey) async {
    bool confirm = await showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: const Text("Delete History?"),
        content: const Text("Are you sure you want to delete this due history entry?"),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text("CANCEL", style: TextStyle(color: Colors.grey))
    ...
    if (confirm) {
      await DBHelper.deleteCustomerDueHistory(widget.c.id!, timestampKey);

      BackupService.autoBackupOnAppOpen(); // <-- ADD THIS LINE

      final updatedHistory = await DBHelper.getCustomerDueHistory(widget.c.id!);
      setState(() {
        _dueHistory = updatedHistory;
      });
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("History entry deleted"), backgroundColor: Colors.redAccent));
      }
    }
  }
  ...
  // Add this controller at the top of your state class variables
  ...
  Future<void> _shareCustomerDetails() async {
    // Start with all available items selected by default
    List<String> selectedShareItems = List.from(imgs);
          ...
          builder: (context, setModalState) {
            return Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text("Select items to share", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 10),
                  Expanded(
                    child: imgs.isEmpty
                    ? const Center(child: Text("No images or receipts available."))
                    : ListView.builder(
                      itemCount: imgs.length,
                      itemBuilder: (context, index) {
                        String item = imgs[index];
                        bool isReceipt = _isReceipt(item);
                        bool isSelected = selectedShareItems.contains(item);
                        ...
                        return CheckboxListTile(
                          ...
                          onChanged: (bool? val) {
                            setModalState(() {
                              if (val == true) {
                                selectedShareItems.add(item);
                              } else {
                                selectedShareItems.remove(item);
                              }
                            });
                          },
                          title: Text(isReceipt ? "Receipt" : "Physical Image", style: const TextStyle(fontWeight: FontWeight.w600)),
                          subtitle: isReceipt
                          ? Text("Bill ID: ${_parseReceiptData(item)['billId'] ?? ''}")
                          : const Text("Attached photo"),
                          secondary: isReceipt
                          ? const Icon(Icons.receipt_long, color: Colors.teal, size: 36)
                          : ClipRRect(
                            borderRadius: BorderRadius.circular(6),
                            child: Image.file(
                              File(item),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: const Text("SHARE SELECTED", style: TextStyle(fontWeight: FontWeight.bold)),
                      onPressed: () {
                        Navigator.pop(context);
                        _executeShare(selectedShareItems); // Process and share
                      },
                    ),
                  ),
                ],
  ...
  // --- CORE SHARE EXECUTION ---
  Future<void> _executeShare(List<String> selectedItems) async {
    try {
      // Show loading indicator while generating temporary receipt images
      showDialog(
        context: context,
      ...
      for (String item in selectedItems) {
        ...
        if (_isReceipt(item)) {
          // 🛠️ Convert Data to Temporary Image
          XFile? receiptImg = await _generateReceiptImage(item);
          if (receiptImg != null) {
            xFilesToShare.add(receiptImg);
          }
        } else {
          // 📸 Existing Physical File
          File physicalFile = File(item.trim());
          if (await physicalFile.exists()) {
            xFilesToShare.add(XFile(item.trim()));
          }
        }
      }
  ...
  // --- CONVERT RECEIPT DATA TO COMPACT IMAGE ---
  Future<XFile?> _generateReceiptImage(String receiptItem) async {
    final basicData = _parseReceiptData(receiptItem);
    String billId = basicData['billId'] ?? '';
    String basicAmountStr = basicData['amount'] ?? '0';
    String fallbackDate = basicData['date'] ?? '';

    final billData = await _fetchFullReceiptDetails(billId);
    if (billData == null) return null;

    List<dynamic> items = billData['items'] ?? [];
    //String rawDate = billData['date'] ?? fallbackDate;
    String billDate = fallbackDate; // Formats the actual creation date

    // High-density compact receipt widget with proper table & safe logo
      ...
      child: Directionality(
        ...
        child: Container(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              // --- HEADER ROW (SAFE LOGO & BILL INFO) ---
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Row(
                    children: [
                      // Safe circular local image logo
                      Container(
                        width: 32,
                        height: 32,
                        decoration: const BoxDecoration(
                          ...
                          image: DecorationImage(
                            image: AssetImage('assets/images/app_icon.png'), // Replace with your local image asset path
                            fit: BoxFit.cover,
                          ),
                        ),
                      ),
                  ...
                  Column(
                    ...
                    children: [
                      Text("Bill #$billId", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Colors.black87)),
                      Text(billDate, style: const TextStyle(fontSize: 11, color: Colors.grey, fontWeight: FontWeight.w500)),
                    ],
                  ),
                ],
              ),
              ...
              // Customer Details
              Text("Name: ${widget.c.name}, Phone: ${widget.c.phone==''?'Not avilable':widget.c.phone}", style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Colors.black87)),
              const SizedBox(height: 12),

              // --- PROPER COLORED TABLE ---
              Container(
                decoration: BoxDecoration(
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: const Row(
                          children: [
                            Expanded(flex: 5, child: Text("ITEM", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white))),
                            Expanded(flex: 4, child: Text("QTY × RATE", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white))),
                            Expanded(flex: 3, child: Text("AMOUNT", textAlign: TextAlign.right, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.white))),
                          ],
                        ),
                      ),
                      ...
                      Color rowColor = index % 2 == 0 ? Colors.white : Colors.teal.shade50.withOpacity(0.6);
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  boxShadow: [
                    BoxShadow(color: Colors.teal.shade200, blurRadius: 4, offset: const Offset(0, 2))
                  ]
                ),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("Total Bill Amount", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.white)),
                    Text("${_formatMoney(double.tryParse(basicAmountStr) ?? 0.0)} টাকা", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white, fontSize: 15)),
                  ],
                ),
              ),
            ],
    ...
    try {
      ...
      final imagePath = await File('${directory.path}/receipt_share_$billId.png').create();
      await imagePath.writeAsBytes(imageBytes);
      return XFile(imagePath.path);
    } catch (e) {
      debugPrint("Error generating temporary receipt image: $e");
      return null;
    }
  }
  ...
  Future<void> _refreshCustomerData() async {
    ...
    try {
      ...
      // 🔴 ADD THIS: Fetch due history
      final dueHistory = await DBHelper.getCustomerDueHistory(widget.c.id!);
      ...
      setState(() {
        ...
        _isPinned = pinned;
        _dueHistory = dueHistory; // 🔴 ADD THIS inside setState
      });
  ...
  void _showUpdateDueBottomSheet() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        ...
        builder: (context, setModalState) {
          double actualDue = double.tryParse(widget.c.due) ?? 0;
          double displayDue = actualDue > 0 ? actualDue : 0.0;
          double advanceAmount = actualDue < 0 ? actualDue.abs() : 0.0;
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          if (confirm) {
                            ...
                            // 🔴 2. ADD THIS LINE: Refreshes the page data instantly (including history)
                            ...
                            if (sendSMS && widget.c.phone.isNotEmpty) {
                              ...
                              if (status == "active") {
                                ...
                                msg = "Dear ${widget.c.name}\nYour due amount has been updated.\nNew Due Balance is Rs. $newAmount\n\n- Asha Grill House\nView details: https://ashagrillhouse.github.io/site/bill.html?q=${widget.c.phone.replaceAll(RegExp(r'\D'), '').substring(widget.c.phone.replaceAll(RegExp(r'\D'), '').length - 10)}";
                              }
                              else {
                  ...
                  children: [
                    ...
                    if (advanceAmount > 0)
                          ...
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  const Text("অ্যাডভান্স জমা আছে", style: TextStyle(fontSize: 12, color: Colors.grey)),
                                  const SizedBox(height: 4),
                                  Text(
                                    "${_formatMoney(advanceAmount)} টাকা",
                      ...
                      if (advanceAmount > 0)
                        ...
                        // 🔴 --- NEW DUE HISTORY SECTION ---
                        if (_dueHistory.isNotEmpty) ...[
                          InkWell(
                            onTap: () {
                              setState(() {
                                _showDueHistory = !_showDueHistory;
                              });
                            },
                            child: Padding(
                              ...
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                children: [
                                  const Text("Due Update History", style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.black54)),
                                  Icon(
                                    _showDueHistory ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                          ...
                          if (_showDueHistory)
                            ListView.builder(
                              ...
                              itemCount: _dueHistory.length,
                              itemBuilder: (context, index) {
                                String dateKey = _dueHistory.keys.elementAt(_dueHistory.length - 1 - index);
                                double parsedAmount = double.tryParse(_dueHistory[dateKey].toString()) ?? 0.0;
                                String formattedAmount = parsedAmount.toStringAsFixed(2);

                                return ListTile(
                                  dense: true,
                                  contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 0),
                                  leading: const Icon(Icons.history, color: Colors.blue, size: 18),
                                  title: Text(
                                    "Updated to: $formattedAmount ₹",
                                    style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold)
                                  ),
                                  subtitle: Text(dateKey, style: const TextStyle(fontSize: 12, color: Colors.grey)),
                                  trailing: IconButton(
                                    icon: const Icon(Icons.delete_outline, color: Colors.red, size: 20),
                                    onPressed: () => _deleteDueHistory(dateKey),
                                  ),
                                );
                              },
                            ),
                    ...
                    rowItem(
                      ...
                      Colors.purple,
                      _formatDateString(widget.c.createdAt),
                      subtitle: "Added On"
                    ),                  ],
                ),
              ),
...
? GridView.builder(
  ...
  itemBuilder: (_, i) {
    final String currentItem = imgs[i];
    final bool isReceiptItem = _isReceipt(currentItem);
    ...
    //       onTap: () {
    //         if (isReceiptItem) {
    //           _showReceiptDialog(context, currentItem);
    //         } else {
    //           showDialog(
    //             context: context,
    //             builder: (_) => Dialog(
    ...
    //       ? Builder(
    //         builder: (context) {
    //           // Parse the data once for this tile
    //           final parsedData = _parseReceiptData(currentItem);
    //           double receiptAmt = double.tryParse(parsedData['amount'] ?? '0') ?? 0.0;
    //
    //           return Container(
    //             width: double.infinity,
    //             height: double.infinity,
    ...
    //                 Text(
    //                   "${_formatMoney(receiptAmt)} ₹", // Correctly shows receipt amount
    //                   style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12),
    //                 ),
    //                 const SizedBox(height: 1),
    //                 Text(
    //                   parsedData['date'] ?? '', // Correctly shows receipt date
    //                   maxLines: 1,
    //                   overflow: TextOverflow.ellipsis,
    ...
    return Stack(
      children: [
        ...
        // DELETE BUTTON OVERLAY (Shows only if NOT a receipt, no curly braces)
        if (!isReceiptItem)
          Positioned(
            top: 4,
            right: 4,
            child: GestureDetector(
              ...
              // 💬 MESSAGE HISTORY HEADER WITH DROPDOWN TOGGLE
              const SizedBox(height: 30),
              InkWell(
                borderRadius: BorderRadius.circular(8),
                onTap: () {
                  setState(() {
                    _showMessageHistory = !_showMessageHistory;
                  });
                },
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      Row(
                        children: [
                          const Icon(Icons.history_edu_rounded, color: Colors.blueGrey, size: 22),
                          const SizedBox(width: 8),
                          Text(
                      ...
                      Icon(
                        _showMessageHistory ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                        color: Colors.black54,
                      ),
                    ],
                  ),
              ...
              // 💬 MESSAGE LIST (Toggles visibility based on _showMessageHistory)
              if (_showMessageHistory)
                    ...
                    return Container(
                      ...
                      child: Column(
                        ...
                        children: [
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  if (confirm) {
                                    ...
                                    await _refreshCustomerData(); // Updates the UI instantly
                                  }
                                },
                          ...
                          // Updated: Linkify widget added here to handle clickable URLs
                          Linkify(
                            onOpen: (link) async {
