...
class _MeasurementPageState extends State<MeasurementPage> {
  ...
  Future<void> _deleteMeasurement(Measurement m) async {
    ...
    if (confirm) {
      // 1. Delete associated price agreements
      await DBHelper.deletePriceAgreement(m.id!);

      // 2. 🔥 THE FIX: Delete all orphaned detailed measurements tied to this profile!
      await DBHelper.executeRawQuery("DELETE FROM measurements WHERE customerId = ${m.id}");

      // 3. Delete the parent profile from tempcustomer
      await DBHelper.deleteMeasurement(m.id!);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Measurement record completely deleted."))
        );
      }
    }
  }
